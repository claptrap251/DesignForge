...
export default function ShareDialog({ projectId, open, onClose }: ShareDialogProps) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center">
      ...
      <div className="relative z-10 w-full max-w-lg rounded-xl p-6 shadow-xl" style={{ backgroundColor: 'var(--bg-page)' }}>
        ...
        <div className="mt-4">
          <h3 className="text-sm font-medium" style={{ color: 'var(--text-secondary)' }}>Create New Link</h3>
          <div className="mt-2 space-y-2">
            <input
              ...
              className="block w-full rounded-lg border px-3 py-2 text-sm shadow-sm placeholder:text-gray-400 focus:outline-none"
            ...
            <div className="flex gap-2">
              <input
                ...
                min="1"
                className="block flex-1 rounded-lg border px-3 py-2 text-sm shadow-sm placeholder:text-gray-400 focus:outline-none"
                style={{ borderColor: 'var(--border-medium)', backgroundColor: 'var(--bg-page)', color: 'var(--text-primary)' }}
              />
              <button
                onClick={createLink}
                disabled={creating}
                className="rounded-lg px-4 py-2 text-sm font-medium text-white focus:outline-none disabled:opacity-50"
        ...
        <div className="mt-6">
          <h3 className="text-sm font-medium" style={{ color: 'var(--text-secondary)' }}>Existing Links</h3>
          {loading ? (
            <div className="py-4 text-center text-sm" style={{ color: 'var(--text-tertiary)' }}>Loading...</div>
          ) : links.length === 0 ? (
            <div className="py-4 text-center text-sm" style={{ color: 'var(--text-tertiary)' }}>No share links yet</div>
          ) : (
            <div className="mt-2 space-y-2">
              {links.map((link) => (
                <div
